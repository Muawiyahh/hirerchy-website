...
export default function Testimonials({
  ...
  const d = tone === "navy";
  return (
    <Section id="testimonials" tone={tone} className={d ? "" : "border-t border-border/60"}>
      {withHeading && (
        <Reveal>
          <Heading
            center
            ...
            >
              ...
              <blockquote
                className={`flex-1 text-sm leading-relaxed ${
                  d ? "text-white/85" : "text-ink/90"
                }`}
              >
                “{t.quote}”
              </blockquote>
              <figcaption
                className={`mt-5 border-t pt-4 ${d ? "border-white/10" : "border-border"}`}
              >
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <div className={`text-sm font-semibold ${d ? "text-white" : "text-ink"}`}>
                      {t.name}
                    </div>
                    <div className={`text-xs ${d ? "text-success-light" : "text-success"}`}>
                      {t.detail}
                    </div>
                  </div>
                  {t.linkedin && (
